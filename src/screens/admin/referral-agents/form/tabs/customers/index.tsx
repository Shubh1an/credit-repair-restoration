...
import { EnumScreens, ReferralCustomerTypes } from '../../../../../../models/enums';
import AuthService from '../../../../../../core/services/auth.service';
import { getReferralAgentCustomers } from '../../../../../../actions/referral.actions';
...
const mapDispatchToProps = (dispatch: any) => {
    return bindActionCreators({
        getReferralAgentCustomers
    }, dispatch);
}
export const ReferralAgentsPrimaryCustomersComponent = connect(mapStateToProps, mapDispatchToProps)((
    (props: {
        agentId?: string,
        type: ReferralCustomerTypes,
        getReferralAgentCustomers: any
    }) => {
        ...
        const loadCustomers = () => {
            setLoading(true);
            props.getReferralAgentCustomers(props.agentId, props.type, axiosSource)
                .then((result: any[]) => {
                    setLoading(false);
                    setCustomers(result || []);
                })
        ...
        return (
            <DashboardWidget title={(props?.type === ReferralCustomerTypes.Primary ? 'Primary' : 'Associated') + ' Clients ' + (!loading ? `(${customers?.length})` : '')} isLoading={loading} allowFullscreen={true} allowMaximize={true} allowMinimize={true} reload={false} >
                                    ...
                                    customers.length ?
                                        ...
                                        <tr>
                                            <td colSpan={6} className="text-center text-danger" style={{ height: '150px' }}>
                                                {!loading && <i>No {(props?.type === ReferralCustomerTypes.Primary ? 'Primary' : 'Associated')} Clients available.</i>}
                                            </td>
                                        </tr>
                                }
                            </tbody>
