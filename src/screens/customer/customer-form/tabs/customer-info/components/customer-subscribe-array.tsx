...
import { subscribeToArrayCustomer, getSubscribeToArrayCustomer } from '../../../../../../actions/customers.actions';
...
const mapDispatchToProps = (dispatch: any) => {
    return bindActionCreators({
        subscribeToArrayCustomer,
        getSubscribeToArrayCustomer
    }, dispatch);
}
export const CustomerSubscribeToArray = connect(null, mapDispatchToProps)((props: {
    cid: string, subscribeToArrayCustomer: any,
    getSubscribeToArrayCustomer: any
}) => {
    ...
    const onCustomerSubscribeArray = async () => {
        let result = await confirm({
            title: 'Subscribe Client for Credit Reports?',
            message: 'Are you sure you would like to subscribe your client for Credit Reports?',
            confirmText: "YES",
        ...
        if (result) {
            ...
            props?.subscribeToArrayCustomer(props?.cid, axiosSource)
                .then((result: any) => {
                    setIsSubscribing(false);
                    if (result) {
                        toastr.success('Client Subscribed successfully!!');
                        loadStatus();
                    } else {
    ...
    const loadStatus = () => {
        setIsSubscribing(true);
        props?.getSubscribeToArrayCustomer(props?.cid, axiosSource)
            .then((result: any) => {
                setIsSubscribing(false);
